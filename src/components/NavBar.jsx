  ...
  return (
    <div className='flex justify-between items-center w-full h-20 px-4
     text-white bg-black fixed'>
        <div>
            <h1 className='text-5xl font-signature ml-2'>Wendell</h1>
        </div>
        ...
        {links.map(({id ,link}) => {
            ...
            font-medium text-gray-500 hover:scale-105 duration-200'>{link}
            </li>
        )})}
        </ul>
        ...
        {nav && (
            ...
            left-0 w-full h-full h-screen bg-gradient-to-b from-black to-gray-800
            text-gray-500'>
                {links.map(({ id, link }) => {
                    return (    
                        <li key={id} className='px-4 cursor-pointer capitalize
